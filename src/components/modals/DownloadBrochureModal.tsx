...
type FormErrors = Partial<Record<keyof BrochureLeadPayload | "submit", string>>

const initialForm: BrochureLeadPayload = {
  fullName: "",
  email: "",
  countryCode: "+91",
  phone: "",
...
function triggerBrochureDownload() {
  ...
  link.download = "NeuralVarsity-Program-Brochure.pdf"
  link.rel = "noopener"
...
export const DownloadBrochureModal = memo(function DownloadBrochureModal({
  isOpen,
  onClose,
  onSuccess,
  returnFocusRef,
  ...
  const [form, setForm] = useState<BrochureLeadPayload>(initialForm)
  const [errors, setErrors] = useState<FormErrors>({})
  const [isSubmitting, setIsSubmitting] = useState(false)

  const updateField = useCallback(
    <K extends keyof BrochureLeadPayload>(key: K, value: BrochureLeadPayload[K]) => {
  ...
  const validateForm = useCallback(() => {
    const nextErrors: FormErrors = {
      ...
      city: validateRequired(form.city, "City / Location") ?? undefined,
      qualification:
  ...
  return (
    ...
    >
      <ModalFormLayout
        ...
        description="Get the full program brochure with curriculum, projects, outcomes, and cohort details."
        onClose={onClose}
        formId="brochure-form"
      ...
      >
        ...
        <label className="flex items-start gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <span className="text-sm leading-relaxed text-zinc-300">
            I agree to the Terms & Conditions and consent to being contacted about
            NeuralVarsity programs.
